...
- Fix the wizard's instance lifecycle: a running tray is now terminated by pid (SIGTERM, graceful) instead of an AppleScript quit that could target the wizard itself, and it is quit *before* the `open` re-exec (LaunchServices would otherwise activate the running tray, ignore `--args -setup`, and block forever); the post-setup launch uses `open -n` so it starts a fresh tray instead of activating the exiting wizard
