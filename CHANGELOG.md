...
- Menu-bar icons are now all macOS template images, fixing a black-on-dark glyph while recording on macOS Tahoe. The colored state icons shipped light/dark variants picked from `AppleInterfaceStyle` — the global Light/Dark toggle, which is not what the menu bar follows: Tahoe's glass bar derives its appearance from the wallpaper behind it, per screen, and re-tints live, so a light-mode system with a dark wallpaper got the dark glyph on a dark bar (and the value was cached once per launch, so even a correct read went stale). Templates let AppKit derive the colour from the bar itself, so state is now a badge shape instead of a colour — disc = recording, ring = warning, bar = transcribing — and the Z glyph always renders correctly. Also removes the `defaults read` subprocess fork from startup, which was expensive with a multi-GB model resident. Icons regenerate with `go run ./packaging/mktrayicons`
