...
- macOS feedback tones now play via AudioToolbox System Sound Services instead of an app-managed malgo playback device: each beep is a fire-and-forget call, so the end-beep sounds instantly at recording stop (previously 100–600ms late behind a per-tone CoreAudio device rebuild that also stalled the run loop and contended with record start). Tones are still synthesized in code and serialized to small WAVs in the cache dir at startup; they follow the system sound-effects output and alert volume
