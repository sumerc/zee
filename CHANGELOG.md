...
- The two on-device providers now share one implementation: the load/swap/mutex lifecycle and the PCM session live in `transcriber/local.go` and `local_session.go`, with `parakeet.go` and `whisper.go` reduced to registry glue plus a small engine adapter. `IsLocal()` recognises both, so hints and streaming are correctly greyed out for Whisper too
...
- The default mic tail-wait drops 100 ms → 50 ms. It is dead time inside the felt latency above (the mic is held open after release so a fast keyup can't clip the last word), and 50 ms still covers the keyup. Existing installs keep whatever `tail_wait_ms` their `config.json` already records — the default only applies when the field is absent, so change it via Settings → Edit Settings… to pick up the new value
