...
- Parakeet v3 multilingual is back as an opt-in download (643 MB, not pre-fetched — the install payload is unchanged). Benched on Metal it transcribes dictation-length clips in 30–40 ms on M5 (8–17× faster than local Whisper; ~0.5 s vs ~2 s on M1-class machines), which un-does the models-v2 assumption that Whisper strictly replaces it. Whisper stays the multilingual default and the coverage engine (~99 languages vs 25). Saved configs that still name `parakeet-v3-multi` resolve to it again instead of migrating to Whisper; configs already migrated stay put
- Local model labels are now role-based — "English — fastest (Parakeet 110M)", "Multilingual — fast (Parakeet v3, 25 languages)", "Multilingual — most languages (Whisper, 99)" — and both local engines render under a single "Local" heading in the tray's Model menu instead of separate "Local (Parakeet)" / "Local (Whisper)" groups. Users pick a role, not an engine; the model name rides along in parentheses and quantization is never mentioned
- Default mic tail-wait halved back to 50 ms (from 100). The earlier revert to 100 blamed tail clipping on the shorter wait, but that clipping matched the whisper `no_timestamps` tail-drop bug since fixed — so the felt-latency win is back on trial. Existing configs keep their saved `tail_wait_ms`
- The recording overlay no longer floats mid-screen after a monitor change. Its position, notch height, and backing scale were computed once, on first show, from whichever display was primary at the time — unplugging an external setup left the panel at coordinates that now landed in the middle of the laptop screen. The screen-dependent geometry is recomputed on every show, so the overlay re-centres under the current primary display's menu bar (and adapts to notch vs. no-notch when moving between displays)

- `hints.txt` now reaches local Whisper, not just the cloud providers. The same comma-separated string that rides along as `prompt` on Groq/OpenAI/Mistral/ElevenLabs is fed to whisper.cpp as `initial_prompt`, with `carry_initial_prompt` so the vocabulary stays pinned to the front of *every* 30 s window instead of being diluted out of the rolling context after the first one. Measured on two saved Turkish dictations: "hangi **app'leri**" → "hangi **API'leri**", and "GitHub **Işığlarına**" → "GitHub **issue'larına**". Parakeet still ignores hints — a greedy CTC/TDT decode has no prompt to bias — so the tray's hints entry is now gated on a per-engine capability (`transcriber.SupportsHints`) rather than on local-vs-cloud. Note the prompt's *style* bleeds into the output, exactly as it does on the cloud providers: an all-lowercase hints file pushes the transcript toward lowercase, and a capitalised one toward mid-sentence capitals — worth keeping hints.txt written the way you want the text to look
- Local Whisper no longer drops the end of long dictations. `whisper_full` ran with `no_timestamps = true` (we only ever wanted text), which selects a decoder path that advances a fixed 30 s per window and skips the retry-on-failed-decode branch — so whatever the model fails to emit inside a window is lost for good, with no way to recover it. On a 131 s recording the last 11 s of speech disappeared and were replaced by a hallucinated sentence; across the whole file 11% of the transcript was missing (1593 vs 1792 chars), not just the tail. Timestamps are now left enabled — nothing is printed and the wrapper joins segment text exactly as before, so the returned string is unchanged in shape, only complete. Speed is a wash (mean of 3, per clip): the clip that was failing got *faster*, 4.24 → 3.57 s, because the bad decode was burning temperature-fallback retries; a clean 160 s clip costs ~8%, 3.78 → 4.08 s; a 27 s clip is unchanged. Upstream bug: whisper.cpp#2186
- Local Whisper is the new offline multilingual engine (`whisper-turbo-q5`, large-v3 turbo q5_0, 547 MB), replacing the Parakeet v3 multilingual model. It defaults to **auto-detect** rather than a fixed language: whisper's language setting hard-forces the start-of-transcript token, so dictating Turkish while pinned to English garbles the text rather than merely mislabelling it — auto-detect is the only mode that survives switching language mid-sentence, at the cost of one extra encoder pass (~260 ms). Parakeet 110M stays the English fast path and the startup default (4–9× faster). The local registry is now exactly two models with two roles — `parakeet-v2-en-large` was retired along with v3, since the 110m already covers English — and both are pre-fetched; the payload shrank (942 MB → 814 MB) since q5_0 measured as a wash against f16 on both latency and transcription quality. Saved selections migrate on upgrade: `parakeet-v3-multi` → Whisper (keeping multilingual), `parakeet-v2-en-large` → 110m (keeping English); older app builds are untouched (they pin the still-published `models-v1` release)
- whisper.cpp (v1.9.1) links the *same* patched ggml that parakeet.cpp builds, so one ggml lives in the process. `make whisper-lib` installs parakeet's ggml to a local prefix and builds `libwhisper.a` against it; without that, whisper would compile against its own vendored headers while linking parakeet's archives — silent struct-layout corruption rather than a link error. A test pins the ggml commit so a parakeet.cpp bump fails loudly instead of breaking the whisper build cryptically
- The two on-device providers now share one implementation: the load/swap/mutex lifecycle and the PCM session live in `transcriber/local.go` and `local_session.go`, with `parakeet.go` and `whisper.go` reduced to registry glue plus a small engine adapter. `IsLocal()` recognises both, so streaming and encoding are correctly greyed out for Whisper too
- `make bench-local` now covers every local engine, not just Parakeet, loading each model with its own backend. Whisper is timed twice per clip — at its shipped auto-detect default and at a fixed language — because only the fixed-language number is comparable with Parakeet's single encoder pass
- Setup's live mic test now uses Whisper with auto-detect — speak any language — and constructs the engine *before* recording, so the model load and first-run GPU pipeline compile happen while you're still talking instead of stalling silently after "Heard:". The one-time warm-up is thus paid during setup, not on the first real dictation; retries reuse the loaded model instead of reloading per attempt. Parakeet falls back in when the whisper model is missing and its download declined. Parakeet remains the default active provider on a fresh install
- The setup providers menu now treats both local engines uniformly: Whisper shows "offline, ready/no key needed" instead of the cloud-provider "key not set", selecting it offers its model download, and the mic-test tick lands on whichever engine actually verified
- `DMG_PATH=` dev installs now read the model manifest from the local checkout instead of main, so a branch's model set installs consistently before it merges; `curl | bash` installs still read main's manifest as before
- Fix a dialog that appeared out of nowhere: "Can't change the language while recording or transcribing" was shown for *derived* language changes (a model-constraint fallback, a config reload, the language list being rebuilt), not just user menu clicks. The tray already distinguished the two, but the guard ran before checking. Derived changes now apply silently — they are a string assignment, and the in-flight session has already captured its language
- Menu-bar state icons are now a bare coloured dot — red recording, orange hearing-nothing, blue transcribing — instead of the Z glyph plus a badge, fixing a black-on-dark glyph while recording on macOS Tahoe. The glyph variant used to be picked from `AppleInterfaceStyle`, the global Light/Dark toggle, which is not what the menu bar follows: Tahoe's glass bar derives its appearance from the wallpaper behind it, per screen, and re-tints live, so a light-mode system with a dark wallpaper got the dark glyph on a dark bar (and the value was cached once per launch, so even a correct read went stale). A saturated dot is legible on either bar, so the light/dark variants and all appearance-detection code are gone — no `defaults read` fork at startup (costly with a multi-GB model resident), and no cgo/KVO watcher. Idle is unchanged: the Z as a template image, tinted by AppKit. Icons regenerate with `go run ./packaging/mktrayicons`
- New `felt_latency` log line measures the one delay the user actually perceives: `release_to_text_ms`, from hotkey release (or silence auto-close) to the text being pasted/copied. It spans the whole tail — mic tail-wait, device stop, encode, inference, network, paste — which no existing metric covered: `press_to_record_ms` measures the front of the press and `total_ms`/`inference_ms` start once the audio is already in hand, so a stall in between was invisible. Emitted once per transcription for batch *and* streaming providers, unlike the per-mode metrics lines
- `go test` no longer pops real macOS dialogs. `main_test.go` drives `tryStartSession` with `isRecording` set on purpose, which reached `alert.Warn` → `osascript display dialog` — so running the suite blasted "Already recording or transcribing." onto the screen, looking exactly like the app misfiring on its own, and would hang CI on a dialog nobody can answer. Suppressed in the `alert` package (keyed off the `.test` binary name, or `ZEE_NO_DIALOGS`), so every call site is covered
- Guard denials are now logged with the caller and engine state. They previously wrote nothing at all, which made a dialog that appeared without user input impossible to trace
...
- The mic now stays open ~100ms after the hotkey is released so a fast keyup no longer clips the last word (the recording ended mid-speech, and the transcriber then guessed at the truncated word). This tail-wait previously ran only for streaming providers; it now applies to every provider, including the local models. Configurable via `tail_wait_ms` in `config.json` (default 100; 0 disables it)
