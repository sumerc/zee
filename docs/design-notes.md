...
## Why menu-bar state is a shape, not a colour

All tray icons are macOS **template** images (alpha only; AppKit tints them to
the bar). State is therefore a badge shape — `●` recording, `○` warning,
`▬` transcribing, bare glyph idle — regenerated by
`go run ./packaging/mktrayicons`.

The colored icons this replaced shipped two variants each (white glyph for a
dark bar, dark glyph for a light one) chosen from `AppleInterfaceStyle`. That
key is the **global Light/Dark toggle**, not the menu bar's appearance. The two
coincided until macOS Tahoe, whose glass bar derives its appearance from the
wallpaper behind it, per screen, and re-tints live — so light mode + dark
wallpaper rendered a dark glyph on a dark bar. The value was also read once per
launch, so even a correct answer went stale on a wallpaper change. Idle never
broke, because it was already a template: the bug was visible only on the
colored states.

Rejected alternatives:
- **Read `NSStatusItem.button.effectiveAppearance`** (the true source, KVO-able).
  `energye/systray` keeps `statusItem` as a private ObjC ivar, so this needs a
  fork of the lib or walking `[NSApp windows]` to find the `NSStatusBarButton`.
- **Template glyph + `button.contentTintColor`** — keeps red/blue and still lets
  the system solve light/dark, but tints the *whole* glyph (a red Z, not a red
  dot) and needs the same private button.

Cost of the shape approach: the state marks are ~5.5 pt, so they must differ in
silhouette rather than detail — disc/ring/bar clear that bar, finer distinctions
would not. Apple's Tahoe HIG pushes monochrome template status items for the
same reason, so this follows the platform rather than fighting it.
