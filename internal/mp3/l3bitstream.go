...
func (enc *Encoder) formatBitstream() {
	...
	enc.encodeSideInfo()
	enc.encodeMainData()
	// Write any remaining bits that didn't fit in Part2_3Length (max 4095)
	if enc.sideInfo.ReservoirDrain > 0 {
		stuffingWords := enc.sideInfo.ReservoirDrain / 32
		remainingBits := enc.sideInfo.ReservoirDrain % 32
		for stuffingWords > 0 {
